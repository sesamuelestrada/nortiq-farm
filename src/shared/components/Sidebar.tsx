...
export function Sidebar({ userName, initialHoyCount }: Props) {
  ...
  return (
    <aside className="hidden md:flex h-screen w-64 flex-col border-r border-gray-200/60 dark:border-gray-800/60 bg-white dark:bg-gray-950">
      {/* Logo */}
      <div className="flex items-center gap-3 border-b border-gray-200/60 dark:border-gray-800/60 px-5 py-4">
        ...
        <div>
          <p className="text-sm font-semibold text-gray-900 dark:text-white">Nortiq</p>
          <p className="text-xs text-gray-400 dark:text-gray-500">Mantenimiento</p>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? hasCount && isHoy
                    ? 'bg-red-500/10 text-red-600 dark:text-red-400 border-l-2 border-l-red-500 pl-[10px]'
                    : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-l-2 border-l-emerald-500 pl-[10px]'
                  : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-800 dark:hover:text-gray-200 border-l-2 border-l-transparent pl-[10px]'
              )}
            >
              <Icon className={cn(
                'h-4 w-4 flex-shrink-0',
                isActive
                  ? hasCount && isHoy ? 'text-red-500 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'
                  : hasCount && isHoy ? 'text-red-500' : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-gray-300'
              )} />
      ...
      <div className="border-t border-gray-200/60 dark:border-gray-800/60 p-3 space-y-1">
        {userName && (
          <div className="flex items-center gap-3 px-3 py-2 text-sm text-gray-500 dark:text-gray-400">
            <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800">
              <User className="h-3.5 w-3.5 text-gray-400" />
            </div>
            <span className="truncate font-medium text-gray-700 dark:text-gray-300">{userName}</span>
          </div>
        )}
        <ThemeToggle />
        <form action={logout}>
