...
const severityConfig = {
  critical: { label: 'Crítico', bg: 'bg-red-50 dark:bg-red-950/30', text: 'text-red-700 dark:text-red-400', border: 'border-red-200 dark:border-red-800/40', dot: 'bg-red-500' },
  high: { label: 'Alto', bg: 'bg-orange-50 dark:bg-orange-950/30', text: 'text-orange-700 dark:text-orange-400', border: 'border-orange-200 dark:border-orange-800/40', dot: 'bg-orange-500' },
  medium: { label: 'Medio', bg: 'bg-amber-50 dark:bg-amber-950/30', text: 'text-amber-700 dark:text-amber-400', border: 'border-amber-200 dark:border-amber-800/40', dot: 'bg-amber-500' },
  low: { label: 'Bajo', bg: 'bg-blue-50 dark:bg-blue-950/30', text: 'text-blue-700 dark:text-blue-400', border: 'border-blue-200 dark:border-blue-800/40', dot: 'bg-blue-400' },
}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="pt-5 pb-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground font-medium">Costo / hora</p>
                <p className="text-xl font-bold text-foreground mt-0.5">
                  {costPerHour != null
        ...
        <Card>
          <CardContent className="pt-5 pb-4">
            <div className="flex items-start gap-3">
              <div className={`flex h-9 w-9 items-center justify-center rounded-xl ${openFindings.length > 0 ? 'bg-red-50 dark:bg-red-950/30' : 'bg-emerald-50 dark:bg-emerald-950/30'}`}>
                <AlertTriangle className={`h-4.5 w-4.5 ${openFindings.length > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`} />
              </div>
              <div>
                <p className="text-xs text-muted-foreground font-medium">Hallazgos abiertos</p>
                <p className={`text-xl font-bold mt-0.5 ${openFindings.length > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                  {openFindings.length}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium text-muted-foreground">Costo por mes (últimos 6 meses)</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          {totalCost === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-6">Sin registros de costo en los últimos 6 meses</p>
          ) : (
      ...
      {openFindings.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Hallazgos abiertos</CardTitle>
          </CardHeader>
